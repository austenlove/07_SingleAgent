"""Hybrid retrieval ported from 03_Advanced_RAG.

Combines ChromaDB dense vectors with BM25 sparse scoring, fuses via RRF, and
applies an LLM-based reranker. Used by the agent's `rag_search` tool.
"""
...
def _embed(texts: list[str]) -> list[list[float]]:
    resp = _openai_client().embeddings.create(model=settings.embedding_model, input=texts)
...
def _sparse_hits(question: str, k: int) -> list[dict[str, Any]]:
    ...
    for idx in indices:
        ...
        out.append({"id": f"bm25::{idx}", "text": text, "meta": meta, "score": float(scores[idx])})
    return out
